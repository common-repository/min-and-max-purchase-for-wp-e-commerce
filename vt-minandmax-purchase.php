<?php
/*
Plugin Name: VarkTech Min and Max Purchase for WP E-Commerce
Plugin URI: http://varktech.com
Description: An e-commerce add-on for WP-E-Commerce, supplying minimum and maximum purchase rule functionality.
Version: 1.06
Author: Vark
...
class VTMAM_Controller{
    ...
    require ( VTMAM_DIRNAME . '/wpsc-integration/vtmam-parent-definitions.php');
 ...
 *************************************************** */
    ...
    require ( VTMAM_DIRNAME . '/wpsc-integration/vtmam-parent-functions.php');
    require ( VTMAM_DIRNAME . '/wpsc-integration/vtmam-parent-cart-validation.php');
  ...
  function vtmam_activation_hook() {
    ...
    global $wp_version;
		if((float)$wp_version < 3.3){
			// delete_option('vtmam_setup_options');
			 wp_die( __('<strong>Looks like you\'re running an older version of WordPress, you need to be running at least WordPress 3.3 to use the Varktech Min and Max Purchase plugin.</strong>', 'vtmam'), __('VT Min and Max Purchase not compatible - WP', 'vtmam'), array('back_link' => true));
			return;
		}
   ...
   if (version_compare(PHP_VERSION, VTMAM_EARLIEST_ALLOWED_PHP_VERSION) < 0) {    //'<0' = 1st value is lower 
    ...
    if(defined('WOOCOMMERCE_VERSION') && (VTMAM_PARENT_PLUGIN_NAME == 'WooCommerce')) { 
      ...
      if( (version_compare(strval($new_version), strval($current_version), '>') == 1) ) {   //'==1' = 2nd value is lower 
  			// delete_option('vtmam_setup_options');
  			 wp_die( __('<strong>Looks like you\'re running an older version of WooCommerce. <br>You need to be running at least ** WooCommerce 3.8 **, to use the Varktech Min and Max Purchase plugin.</strong>', 'vtmam'), __('VT Min and Max Purchase not compatible - WooCommerce', 'vtmam'), array('back_link' => true));
  			return;
  		}
    }   else 
    if (VTMAM_PARENT_PLUGIN_NAME == 'WooCommerce') {
        wp_die( __('<strong>Varktech Min and Max Purchase for WooCommerce requires that WooCommerce be installed and activated.</strong>', 'vtmam'), __('WooCommerce not installed or activated', 'vtmam'), array('back_link' => true));
  			return;
    }
    ...
    if(defined('JIGOSHOP_VERSION') && (VTMAM_PARENT_PLUGIN_NAME == 'JigoShop')) { 
      ...
      if( (version_compare(strval($new_version), strval($current_version), '>') == 1) ) {   //'==1' = 2nd value is lower
  			// delete_option('vtmam_setup_options');
  			 wp_die( __('<strong>Looks like you\'re running an older version of JigoShop. <br>You need to be running at least ** JigoShop 3.8 **, to use the Varktech Min and Max Purchase plugin.</strong>', 'vtmam'), __('VT Min and Max Purchase not compatible - JigoShop', 'vtmam'), array('back_link' => true));
  			return;
  		}
    }  else 
    if (VTMAM_PARENT_PLUGIN_NAME == 'JigoShop') {
        wp_die( __('<strong>Varktech Min and Max Purchase for JigoShop requires that JigoShop be installed and activated.</strong>', 'vtmam'), __('JigoShop not installed or activated', 'vtmam'), array('back_link' => true));
  			return;
    }
